...
\begin{itemize}
    \item \textbf{SonarQube} for initial code smell detection using rule-based static analysis
    \item \textbf{LLM-based agents} built using LangGraph framework for intelligent analysis and refactoring
    \item \textbf{MLflow} for experiment tracking, evaluation, and dataset management
    \item \textbf{Dependency analysis engine} that models positive and negative relationships between code smells
    \item \textbf{Priority calculation system} that determines optimal refactoring sequences based on severity and impact
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{Long Method} (java:S138) - Methods exceeding 60 lines of code
        \item \textbf{Complex Method} (java:S1541) - Methods with high cyclomatic complexity
        \item \textbf{Conditional Complexity} (java:S1067) - Deeply nested conditional expressions
        \item \textbf{Long Parameter List} (java:S107) - Methods with more than 7 parameters
        \item \textbf{God Class} (java:S1200) - Classes coupled to too many other classes
        \item \textbf{Large Class} (java:S110) - Classes with excessive lines of code
        \item \textbf{Duplicated Conditions} (java:S1871) - Identical conditional branches
        \item \textbf{Print Statements} (java:S106) - Direct use of System.out/System.err
    \end{itemize}
...
\begin{itemize}
    \item \textbf{Positive dependencies (PZ)}: Refactoring smell A may simultaneously resolve smell B
    \begin{itemize}
    ...
    \end{itemize}
    \item \textbf{Negative dependencies (NZ)}: Refactoring smell A may introduce smell B
    \begin{itemize}
...
\begin{itemize}
    \item \textbf{Long Method / Complex Method / Conditional Complexity}
    \begin{itemize}
    ...
    \item \textbf{Large Class / God Class}
    \begin{itemize}
    ...
    \item \textbf{Duplicated Conditions}
    \begin{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{Long Method}: Extract Method refactoring
        \item \textbf{Large Class/God Class}: Extract Class refactoring
        \item \textbf{Long Parameter List}: Introduce Parameter Object
        \item \textbf{Duplicated Code}: Extract Method and remove duplication
        \item \textbf{Complex Method}: Decompose conditional logic
    \end{itemize}
...
\begin{itemize}
    \item \textbf{Dataset management}: Creating and versioning datasets from RefactoringMiner data
    \item \textbf{Experiment tracking}: Recording model configurations, parameters, and metrics for each evaluation run
    \item \textbf{Result comparison}: Comparing different approaches (vanilla LLM, RAG-enhanced, hybrid)
    \item \textbf{Reproducibility}: Ensuring experiments can be reproduced with exact configurations
\end{itemize}
...
\begin{itemize}
    \item \textbf{Size}: 547 commits from 188 open-source projects
    \item \textbf{Time range}: Commits between June 8, 2015 and August 7, 2015
    \item \textbf{Content}: Real refactorings performed by developers, including:
    \begin{itemize}
...
\begin{itemize}
    \item \textbf{OrderProcessor.java}: Demonstrates positive dependencies in the ``bad size'' category (Long Method, Complex Method, Duplicated Code, Switch Statement)
    \item \textbf{CustomerDataService.java}: Demonstrates negative dependencies (Long Parameter List vs. Data Class trade-off)
    \item \textbf{ReportGenerator.java}: Demonstrates God Class with multiple interconnected smells
    \item \textbf{PaymentValidator.java}: Demonstrates duplicated conditions and their dependencies
    \item \textbf{ConfigurationManager.java}: Demonstrates complex interplay of multiple smells
\end{itemize}
...
\begin{itemize}
    \item \textbf{WMC (Weighted Methods per Class)}: Sum of cyclomatic complexities of all methods in a class
    \item \textbf{CBO (Coupling Between Objects)}: Number of classes to which a class is coupled
    \item \textbf{RFC (Response For a Class)}: Number of methods that can potentially be executed in response to a message received by an object
    \item \textbf{LCOM (Lack of Cohesion of Methods)}: Measures how methods and attributes are related
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{LangGraph}: Framework for building multi-agent workflows with state management
    \item \textbf{LangChain}: LLM orchestration and tool integration
    \item \textbf{LiteLLM}: Unified interface for multiple LLM providers (OpenAI, Anthropic, Google)
    \item \textbf{MLflow}: Experiment tracking and dataset management
    \item \textbf{SonarQube}: Static code analysis engine
    \item \textbf{NetworkX}: Graph analysis and dependency modeling
    \item \textbf{Matplotlib}: Visualization of dependency graphs
    \item \textbf{Pydantic}: Data validation and structured outputs from LLMs
\end{itemize}

The modular architecture allows different LLM models to be used for different agents based on task complexity:
\begin{itemize}
    \item \textbf{GPT-4o-mini}: Default model for most agents (cost-effective)
    \item \textbf{GPT-4o}: For complex refactoring tasks requiring deeper reasoning
    \item \textbf{Claude Sonnet}: Alternative for specific agent implementations
    \item \textbf{Gemini 2.5}: Used in RAG experiments for code understanding
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Initial scan}: Agent 1 analyzes a commit using SonarQube and identifies code smells
    \item \textbf{Test verification}: Agent 2 checks test coverage for affected methods
    \item \textbf{Dependency analysis}: Agent 4 analyzes smell dependencies and calculates PZ scores
    \item \textbf{Priority determination}: Agent 4 generates refactoring sequence based on maximum PZ strategy
    \item \textbf{Visualization}: System generates dependency graphs and priority visualizations
    \item \textbf{Refactoring execution}: Agent 5 applies refactorings in priority order
    \item \textbf{Verification}: Agent 6 runs tests after each refactoring to ensure correctness
    \item \textbf{Metrics evaluation}: System calculates code quality metrics before and after
    \item \textbf{Result logging}: MLflow records all steps, decisions, and outcomes
\end{enumerate}
