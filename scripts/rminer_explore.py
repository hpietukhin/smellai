...
Typical usage (with uv):
    ...
    uv run python scripts/rminer_explore.py /path/to/data.json

    uv run python scripts/rminer_explore.py /path/to/data.json --max-commits 20
...
from models.refactoring import CommitCluster, RefactoringStats, RMinerCommit
...
def _analyze_clusters(
    repo_groups: Dict[str, List[RMinerCommit]], *, max_id_gap: int, min_cluster_size: int
) -> List[CommitCluster]:
    """
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    LOGGER.info(
        f"Loaded {len(commits)} commits from {len(unique_repos)} repositories"
    )
    ...
    all_clusters = _analyze_clusters(
        selected_repos, max_id_gap=args.max_id_gap, min_cluster_size=args.min_cluster_size
    )
