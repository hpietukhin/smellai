...
Usage:
    # Create dataset
    uv run cli/datasets/rminer_dataset.py --manifest rminer_data/manifest.json

    # Dry run (preview records)
    uv run cli/datasets/rminer_dataset.py --manifest rminer_data/manifest.json --dry-run

    # Limit number of pairs
    uv run cli/datasets/rminer_dataset.py --manifest rminer_data/manifest.json --limit 10
"""
...
# Add repo root to sys.path to allow importing infra modules
# Add repo root to sys.path to allow importing smellai modules
REPO_ROOT = Path(__file__).resolve().parent.parent
if str(REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(REPO_ROOT))

try:
    from smellai.infra.sonarqube.commit_scan import scan_commit
except ImportError:
    scan_commit = None
    ...
    for pair in pairs:
        ...
        if scan_commit and sonar_url and sonar_token:
            try:
                ...
                print(f"Warning: SonarQube scan failed for {pair['id']}: {e}", file=sys.stderr)
...
def main() -> int:
    ...
    sonar_cache_dir = Path(args.sonar_cache_dir) if args.sonar_cache_dir else Path("rminer_data/sonar_cache")
